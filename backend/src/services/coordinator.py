    ...
    async def _parallel_rendering(self, segments: List[dict]) -> List[dict]:
        ...
        async def render_single_clip(segment: dict, index: int) -> dict:
            ...
            try:
                ...
                logger.info(
                    "VPI_TEMP_GENERAL_WRITE_TRACE task_id=%s clip_order=%d caller=%s input_path=%s output_dir=%s output_path=%s",
                    self.task_id,
                    index + 1,
                    "coordinator.render_single_clip",
                    str(self.video_path),
                    str(output_dir),
                    str(output_dir / f"clip_{index + 1:02d}.mp4"),
                )
