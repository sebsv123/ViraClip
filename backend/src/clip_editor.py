...
def shape_morph_transition(
    ...
    *quality* controla el segmentador:
    ...
    try:
        ...
        # Escribir frames morph como PNGs y reensamblar con FFmpeg (libx264)
        # NOTA: No usamos cv2.VideoWriter porque su codec "mp4v" (MPEG-4 Part 2)
        # no es H.264. Al concatenar con el resto del video (H.264/AAC), el MP4
        # resultante tiene streams de codecs mixtos que los navegadores no pueden
        # decodificar, causando congelamiento a los ~10 segundos.
        import tempfile
        ...
        for i, f in enumerate(morph_frames):
            cv2.imwrite(str(frames_dir / f"frame_{i:05d}.png"), cv2.cvtColor(f, cv2.COLOR_RGB2BGR))
        cmd = [
            "ffmpeg", "-y",
            "-framerate", str(fps),
